...
<Steps>
  ...
  <Step title="Print your Logs">
    ...
    For general logs with annotated log levels (v1.5+):
    ```cpp
    Serial.print("[INFO] Info level")
    Serial.print("[WARNING] Warning level")
    Serial.print("[ERROR] Error level")
    Serial.print("[DONE] Task complete")
    ```
  </Step>
